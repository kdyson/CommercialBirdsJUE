...
## Need to add a few variables to describe aggregate foraging behavior, etc.
    ...
    ## text.spp (like incidence is) in order to determine # times seen foraging.
        ...
        bird.foraging.incidence.2014 <-
            subset(bird.by.visit.PA, visit.number < 5) %>%
            group_by(., site.name, text.spp) %>%
            summarise(
                proportion.foraging = sum(sweeps.foraging > 0) / 4,
                sweeps.foraging = paste(sweeps.foraging, collapse = ", "),
                visits.seen = paste(as.character(unique(visit.number)),
                                    collapse = ", "),
                PA.foraging = ifelse(proportion.foraging > 0, 1, 0)
        ...
        bird.foraging.incidence.2015 <-
            subset(bird.by.visit.PA, visit.number > 4) %>%
            group_by(., site.name, text.spp) %>%
            summarise(
                proportion.foraging = sum(sweeps.foraging > 0) / 4,
                sweeps.foraging = paste(sweeps.foraging, collapse = ", "),
...
## Matrify bird data
    ...
    # 2024-2015 field season
        matrify.incidence.2014 <-
            ez.matrify(
                filename = incidence,
                species.name = "text.spp",
                site.name = "site.name",
                abundance = "incidence.2014"
            )
        
        matrify.incidence.2014 <-
            matrify.incidence.2014[ , 
                               !(colnames(matrify.incidence.2014) %in%
                                     c("sharp.shinned.hawk", "coopers.hawk", "unknown")) ]
        matrify.incidence.2014$site <- rownames(matrify.incidence.2014)
        matrify.incidence.2014$year <- 2014
        
        
        matrify.incidence.2014_2.clean <- matrify.incidence.2014[, sp.seen_2.clean]
        matrify.incidence.2014_2.clean$site <- rownames(matrify.incidence.2014_2.clean)
        matrify.incidence.2014_2.clean$year <- 2014
        
        matrify.foraging.2014 <-
            ez.matrify(
                filename = bird.foraging.incidence.2014,
                species.name = "text.spp",
                site.name = "site.name",
                abundance = "proportion.foraging"
            )
        ...
        matrify.foraging.2014 <-
          matrify.foraging.2014[, !(
            colnames(matrify.foraging.2014) %in%
              c("sharp.shinned.hawk", "coopers.hawk", "no.birds")
          ) &
            (colSums(matrify.foraging.2014) > 0)]
        
        matrify.foraging.2014$z.site <- rownames(matrify.foraging.2014)
        matrify.foraging.2014$z.year <- 2014
        
        matrify.foraging.2014_2 <- matrify.foraging.2014[, colnames(matrify.foraging.2014) %in% sp.seen_2.clean]
        matrify.foraging.2014_2$z.site <- rownames(matrify.foraging.2014_2)
        matrify.foraging.2014_2$z.year <- 2014

        guild.eff.sp.rich.2014_2.clean <-
          tibble(
            site.name = rownames(matrify.incidence.2014_2.clean),
            year = 2014,
            omnivore.esr = exp(diversity(matrify.incidence.2014_2.clean[,colnames(matrify.incidence.2014_2.clean) %in%
                                                                  bird.guilds$species.name[bird.guilds$diet == "omnivore"]])),
            grainivore.esr = exp(diversity(matrify.incidence.2014_2.clean[, colnames(matrify.incidence.2014_2.clean) %in%
                                                                  bird.guilds$species.name[bird.guilds$diet == "grainivore"]])),
            insectivore.esr = exp(diversity(matrify.incidence.2014_2.clean[, colnames(matrify.incidence.2014_2.clean) %in%
                                                                  bird.guilds$species.name[bird.guilds$diet == "insectivore"]])),
            
            ground.for.esr = exp(diversity(matrify.incidence.2014_2.clean[, colnames(matrify.incidence.2014_2.clean) %in%
                                                                  bird.guilds$species.name[bird.guilds$foraging.substrate ==
                                                                                             "ground"]])),
            treshrb.for.esr = exp(diversity(matrify.incidence.2014_2.clean[, colnames(matrify.incidence.2014_2.clean) %in%
                                                                  bird.guilds$species.name[bird.guilds$foraging.substrate ==
                                                                                             "trees.shrubs"]])),
            generalist.for.esr = exp(diversity(matrify.incidence.2014_2.clean[, colnames(matrify.incidence.2014_2.clean) %in%
                                                                  bird.guilds$species.name[bird.guilds$foraging.substrate ==
                                                                                             "generalist"]])),
            
            mixed.esr = exp(diversity(matrify.incidence.2014_2.clean[, colnames(matrify.incidence.2014_2.clean) %in%
                                                                  bird.guilds$species.name[bird.guilds$forest.preference ==
                                                                                             "mixed"]])),
            conifer.esr = exp(diversity(matrify.incidence.2014_2.clean[, colnames(matrify.incidence.2014_2.clean) %in%
                                                                  bird.guilds$species.name[bird.guilds$forest.preference ==
                                                                                             "conifer"]])),
            open.esr = exp(diversity(matrify.incidence.2014_2.clean[, colnames(matrify.incidence.2014_2.clean) %in%
                                                                  bird.guilds$species.name[bird.guilds$forest.preference ==
                                                                                             "open.none"]]))
          )
        
    # 2015-2016 season
        matrify.incidence.2015 <-
            ez.matrify(
                filename = incidence,
                species.name = "text.spp",
                site.name = "site.name",
                abundance = "incidence.2015"
            )
        
        matrify.incidence.2015 <-
            matrify.incidence.2015[ , 
                               !(colnames(matrify.incidence.2015) %in%
                                     c("sharp.shinned.hawk", "coopers.hawk", "unknown")) ]
        matrify.incidence.2015$site <- rownames(matrify.incidence.2015)
        matrify.incidence.2015$year <- 2015
        
        matrify.incidence.2015_2.clean <- matrify.incidence.2015[, sp.seen_2.clean]
        matrify.incidence.2015_2.clean$site <- rownames(matrify.incidence.2015_2.clean)
        matrify.incidence.2015_2.clean$year <- 2015

        matrify.foraging.2015 <-
            ez.matrify(
                filename = bird.foraging.incidence.2015,
                species.name = "text.spp",
                site.name = "site.name",
                abundance = "proportion.foraging"
            )
        colSums(matrify.foraging.2015)
        
        matrify.foraging.2015 <-
            matrify.foraging.2015[,
                    !(colnames(matrify.foraging.2015) %in%
                    c("sharp.shinned.hawk", "coopers.hawk", "no.birds")) & 
                      (colSums(matrify.foraging.2015) > 0)
            ]
        matrify.foraging.2015$z.site <- rownames(matrify.foraging.2015)
        matrify.foraging.2015$z.year <- 2015
        
        matrify.foraging.2015_2 <-
            matrify.foraging.2015[, colnames(matrify.foraging.2015) %in% 
                                      sp.seen_2.clean]
        matrify.foraging.2015_2$z.site <- rownames(matrify.foraging.2015_2)
        matrify.foraging.2015_2$z.year <- 2015
        
        
      guild.eff.sp.rich.2015_2.clean <-
        tibble(
          site.name = rownames(matrify.incidence.2015_2.clean),
          year = 2015,
          omnivore.esr = exp(diversity(matrify.incidence.2015_2.clean[, colnames(matrify.incidence.2015_2.clean) %in%
                                                                        bird.guilds$species.name[bird.guilds$diet ==
                                                                                                   "omnivore"]])),
          grainivore.esr = exp(diversity(matrify.incidence.2015_2.clean[, colnames(matrify.incidence.2015_2.clean) %in%
                                                                          bird.guilds$species.name[bird.guilds$diet ==
                                                                                                     "grainivore"]])),
          insectivore.esr = exp(diversity(matrify.incidence.2015_2.clean[, colnames(matrify.incidence.2015_2.clean) %in%
                                                                           bird.guilds$species.name[bird.guilds$diet ==
                                                                                                      "insectivore"]])),
          
          ground.for.esr = exp(diversity(matrify.incidence.2015_2.clean[, colnames(matrify.incidence.2015_2.clean) %in%
                                                                          bird.guilds$species.name[bird.guilds$foraging.substrate == "ground"]])),
          treshrb.for.esr = exp(diversity(matrify.incidence.2015_2.clean[, colnames(matrify.incidence.2015_2.clean) %in%
                                                                           bird.guilds$species.name[bird.guilds$foraging.substrate ==
                                                                                                      "trees.shrubs"]])),
          generalist.for.esr = exp(diversity(matrify.incidence.2015_2.clean[, colnames(matrify.incidence.2015_2.clean) %in%
                                                                              bird.guilds$species.name[bird.guilds$foraging.substrate == "generalist"]])),
          
          mixed.esr = exp(diversity(matrify.incidence.2015_2.clean[, colnames(matrify.incidence.2015_2.clean) %in%
                                                                     bird.guilds$species.name[bird.guilds$forest.preference == "mixed"]])),
          conifer.esr = exp(diversity(matrify.incidence.2015_2.clean[, colnames(matrify.incidence.2015_2.clean) %in%
                                                                       bird.guilds$species.name[bird.guilds$forest.preference == "conifer"]])),
          open.esr = exp(diversity(matrify.incidence.2015_2.clean[, colnames(matrify.incidence.2015_2.clean) %in%
                                                                    bird.guilds$species.name[bird.guilds$forest.preference == "open.none"]]))
        )  
        
        
        
matrify.incidence.RM <- rbind(matrify.incidence.2014, matrify.incidence.2015)
matrify.incidence.RM_2.clean <- rbind(matrify.incidence.2014_2.clean,
                                      matrify.incidence.2015_2.clean)

#this is really hacky... oops
# add to 2015
colnames(matrify.foraging.2014) [!(colnames(matrify.foraging.2014) %in% colnames(matrify.foraging.2015))]
# add to 2014
colnames(matrify.foraging.2015) [!(colnames(matrify.foraging.2015) %in% colnames(matrify.foraging.2014))]

matrify.foraging.2014$downy.woodpecker <- 0
matrify.foraging.2014$rock.pigeon <- 0

matrify.foraging.2015$fox.sparrow <- 0
matrify.foraging.2015$house.sparrow <- 0
matrify.foraging.2015$red.crossbill <- 0
matrify.foraging.2015$townsends.warbler <- 0

matrify.foraging.2014 <- matrify.foraging.2014[ , sort(colnames(matrify.foraging.2014))]
matrify.foraging.2015 <- matrify.foraging.2015[ , sort(colnames(matrify.foraging.2015))]
      
matrify.foraging.RM <- rbind(
    matrify.foraging.2014,
    matrify.foraging.2015
)
...
#still hacky
# add to 2015
colnames(matrify.foraging.2014_2) [!(colnames(matrify.foraging.2014_2) %in% colnames(matrify.foraging.2015_2))]
# add to 2014
colnames(matrify.foraging.2015_2) [!(colnames(matrify.foraging.2015_2) %in% colnames(matrify.foraging.2014_2))]

matrify.foraging.2015_2$fox.sparrow <- 0
matrify.foraging.2015_2$house.sparrow <- 0
matrify.foraging.2015_2$red.crossbill <- 0
matrify.foraging.2015_2$townsends.warbler <- 0

matrify.foraging.RM_2 <- rbind(
  matrify.foraging.2014_2[ , sort(colnames(matrify.foraging.2014_2))],
  matrify.foraging.2015_2[ , sort(colnames(matrify.foraging.2015_2))]
)




guild.eff.sp.rich.RM_2.clean <- rbind(guild.eff.sp.rich.2014_2.clean,
                                      guild.eff.sp.rich.2015_2.clean)
...
## Create detection variables collapsed onto site to attach to incidence
        ...
        site.detection.info <- group_by(bird.visit.info, site.name, year) %>%
            summarise(
        ...
        site.detection.info.20 <- group_by(bird.visit.info, site.name) %>%
          summarise(
            dates = paste(unique(visit.date), collapse = ", "),
            proportion.loud.noise = mean(loud.noise),
            visit.start.mean = mean(visit.start),
...
## Merge so that site visit info is attached to PA info.
    ...
    bird.by.visit.PA.info <- merge.data.frame(x = bird.by.visit.PA, y = bird.visit.info,
                                              ...
                                              sort = FALSE)
#Change units
    sample.covariates$hectares <- sample.covariates$acres * 0.40468564
    sample.covariates$landrentperhect <- sample.covariates$landrentperacre / 1000 / 0.40468564
    sample.covariates$MedianHouseholdIncome_B19013e1 <- sample.covariates$MedianHouseholdIncome_B19013e1/1000
    sample.covariates$Proportion_ForeignBorn_B99051e5 <- sample.covariates$Proportion_ForeignBorn_B99051e5 * 100
    sample.covariates$impervious.pct <- sample.covariates$impervious.pct * 100 
    #site.detection.info$Tot.Precip.in.8v.sum <- site.detection.info$Tot.Precip.in.8v.sum * 2.54
    #site.detection.info.20$Tot.Precip.in.8v.sum <- site.detection.info.20$Tot.Precip.in.8v.sum * 2.54
    sample.covariates$tree.orn.dens <- sample.covariates$tree.orn.dens / 0.40468564
    sample.covariates$conifer.nat.dens <- sample.covariates$conifer.nat.dens / 0.40468564
    sample.covariates$shrub.nat.dens <- sample.covariates$shrub.nat.dens / 0.40468564
    sample.covariates$shrub.orn.dens <- sample.covariates$shrub.orn.dens / 0.40468564
    sample.covariates$TV_mean <- sample.covariates$TV_mean * 100
    sample.covariates$SMV_mean <- sample.covariates$SMV_mean * 100
    
    

# And merge with sample variables:
    sample.covariates.20 <-
      left_join(sample.covariates ,
                vegetation.clusters[,-6],
                c("SiteName" = "SiteName")) %>%
      left_join(. , site.detection.info.20, c("SiteName" = "site.name"))
    
    sample.covariates <-
      left_join(sample.covariates,
                vegetation.clusters[,-6],
                c("SiteName" = "SiteName")) %>%
      right_join(. , site.detection.info, c("SiteName" = "site.name"))
    

    sample.covariates$proportion.loud.noise <- sample.covariates$proportion.loud.noise * 100
    sample.covariates.20$proportion.loud.noise <- sample.covariates.20$proportion.loud.noise * 100
    sample.covariates$clouds.76.100.proportion <- sample.covariates$clouds.76.100.proportion * 100    
    sample.covariates.20$clouds.76.100.proportion <- sample.covariates.20$clouds.76.100.proportion * 100    
